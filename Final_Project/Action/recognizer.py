...
def framewise_recognize(pose, pretrained_model):
    frame, joints, bboxes, xcenter = pose[0], pose[1], pose[2], pose[3]
    joints_norm_per_frame = np.array(pose[-1])#720p下的所有关节点坐标(相对坐标)
...
def get_pose_dict(pose,cur_pose,frame_count,cur_box):
    cur_box.clear()
    frame, joints, bboxes, xcenter = pose[0], pose[1], pose[2], pose[3]
    joints_norm_per_frame = np.array(pose[-1])#720p下的所有关节点坐标(相对坐标)
    ...
    if bboxes:
        ...
        for d in trk_result:
            ...
            try:
                ...
                # 若当前帧无human，默认j=0（无效）
                j = 0
            if joints_norm_per_frame.size > 0 and frame_count%5==0:####################################
                tmp=[frame_count]
                tmp.extend(joints_norm_per_frame[j*36:(j+1)*36])
                cur_pose[d[4]]=tmp
                cur_box[d[4]]=[xmin,ymin,xmax,ymax]
            elif joints_norm_per_frame.size > 0:
                cur_box[d[4]]=[xmin,ymin,xmax,ymax]
    return 1
